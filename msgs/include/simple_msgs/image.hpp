...
public:
  ...
  flatbuffers::DetachedBuffer getBufferData() const
  {
    ...
    {
      builder_->Clear();
      auto encoding_string = builder_->CreateString(encoding_);
      ...
      auto origin_vector = builder_->CreateVector(origin_data.data(), origin_data.size());
